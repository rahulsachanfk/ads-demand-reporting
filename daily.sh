...
MBdate=`date -d '-1 day' +%Y-%m-%d | tr -d '\n'`
MBdate=(${MBdate//-/ })
Ryear=${MBdate[0]}
Rmonth=${MBdate[1]}
Rday=${MBdate[2]}
...
START_DATE="${Ryear}-${Rmonth}-${Rday} 00:00:00"
...
filename="billing_${Ryear}_${Rmonth}_${Rday}.xls";

/usr/lib/jvm/jdk-8-oracle-x64/bin/java -jar /home/rahul.sachan/ads-demand-reporting/target/demand_platform-1.0-SNAPSHOT.jar ${Ryear} ${Rmonth} ${Rday}
...
echo "PFA of Brand Campaign billing report for ${Ryear}-${Rmonth}-${Rday}" | mutt -a "/tmp/$filename" -s "Brand Campaign ${Ryear}-${Rmonth}-${Rday} billing report" -- ${address};